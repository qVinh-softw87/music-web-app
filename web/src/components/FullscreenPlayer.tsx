...
export function FullscreenPlayer() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-end gap-3 p-5 pt-6">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center gap-6 px-6 overflow-hidden">
        <img
          src={currentSong?.cover ?? ""}
          alt="Cover"
          className="w-[320px] max-w-[55vw] h-[320px] max-h-[55vw] rounded-xl object-cover shadow-2xl animate-in fade-in zoom-in-95 duration-500"
        />
        <div className="text-center animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150">
          <div className="text-white text-3xl font-extrabold mb-1.5">{currentSong?.title ?? "—"}</div>
          <div className="text-white/70 text-lg font-medium">{currentSong?.artist ?? "—"}</div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-center gap-7">
          <Tooltip content={shuffle ? "Tắt ngẫu nhiên" : "Bật ngẫu nhiên"}>
            <button
              ...
              className="p-1.5 rounded-full btn-tactile relative"
            >
              <svg
                viewBox="0 0 16 16"
                width={20}
              ...
              {shuffle && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[var(--accent)]" />}
            </button>
          </Tooltip>

          <Tooltip content="Quay lại">
            <button type="button" onClick={prev} className="p-1.5 rounded-full text-[#ccc] hover:text-white btn-tactile">
          ...
          <Tooltip content="Tiếp theo">
            <button type="button" onClick={next} className="p-1.5 rounded-full text-[#ccc] hover:text-white btn-tactile">
              <svg viewBox="0 0 16 16" width={22} height={22} fill="currentColor">
          ...
          >
            ...
            >
              ...
              {repeatMode !== "off" && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[var(--accent)]" />}
            </button>
          </Tooltip>
        </div>
